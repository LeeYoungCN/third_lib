include(ExternalProject)

set(CJSON_NAME cjson)
set(CJSON_PREFIX_DIR    ${EXTERNAL_PREFIX_DIR}/${CJSON_NAME})
set(CJSON_SRC_DIR       ${EXTERNAL_CODE_DIR}/${CJSON_NAME})
set(CJSON_BUILD_DIR     ${EXTERNAL_BUILD_DIR}/${CJSON_NAME})
set(CJSON_INSTALL_DIR   ${EXTERNAL_INSTALL_DIR}/${CJSON_NAME})
set(CJSON_STAMP_DIR     ${EXTERNAL_STAMP_DIR}/${CJSON_NAME})

set(CJSON_GIT_URL "https://github.com/DaveGamble/cJSON.git")
set(CJSON_GIT_TAG "v1.7.15")

set(CJSON_CONFIG_FILE ${EXTERNAL_CONFIG_SOURCE_DIR}/${CJSON_NAME}.cmake)
set(CJSON_INSTALL_FILE ${CJSON_NAME}.cmake)

install(FILES ${CJSON_CONFIG_FILE}
        DESTINATION ${EXTERNAL_CONFIG_INSTALL_DIR}
        RENAME ${CJSON_INSTALL_FILE})

if (EXISTS ${CJSON_SRC_DIR})
    ExternalProject_Add(
        ${CJSON_NAME}

        PREFIX      ${CJSON_PREFIX_DIR}
        SOURCE_DIR  ${CJSON_SRC_DIR}
        BINARY_DIR  ${CJSON_BUILD_DIR}
        STAMP_DIR   ${CJSON_STAMP_DIR}

        CMAKE_ARGS
            -DCMAKE_INSTALL_PREFIX=${CJSON_INSTALL_DIR}
            -DCMAKE_GENERATOR=${CMAKE_GENERATOR}
            -DCMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE}
            -DCMAKE_TOOLCHAIN_FILE=${CMAKE_TOOLCHAIN_FILE}
            -DCMAKE_EXPORT_COMPILE_COMMANDS=ON
            -DCMAKE_POLICY_VERSION_MINIMUM=3.10
            -DENABLE_CJSON_TEST=OFF
            -DENABLE_CUSTOM_COMPILER_FLAGS=OFF
    )
else()
    ExternalProject_Add(
        ${CJSON_NAME}

        GIT_REPOSITORY  ${CJSON_GIT_URL}
        GIT_TAG         ${CJSON_GIT_TAG}

        PREFIX          ${CJSON_PREFIX_DIR}
        SOURCE_DIR      ${CJSON_SRC_DIR}
        BINARY_DIR      ${CJSON_BUILD_DIR}
        STAMP_DIR       ${CJSON_STAMP_DIR}

        CMAKE_ARGS
            -DCMAKE_INSTALL_PREFIX=${CJSON_INSTALL_DIR}
            -DCMAKE_GENERATOR=${CMAKE_GENERATOR}
            -DCMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE}
            -DCMAKE_TOOLCHAIN_FILE=${CMAKE_TOOLCHAIN_FILE}
            -DCMAKE_EXPORT_COMPILE_COMMANDS=ON
            -DCMAKE_POLICY_VERSION_MINIMUM=3.10
            -DENABLE_CJSON_TEST=OFF
            -DENABLE_CUSTOM_COMPILER_FLAGS=OFF

        UPDATE_COMMAND ""
    )
endif()
